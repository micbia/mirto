...
void process_timesteps(
    ...
    const double & dec)
    {
        ...
        int num_timesteps = 20;          // number of timesteps
        int num_pix = 50;       // number of l,m pixels
        int num_baseline = 10;       // number of baselines
        double u;
        double v;

        // CUDA Grid size: since 1 block = 1 uv-point, this sets the number of uv-points treated in parallel
        //dim3 gs(NUM_SRC_PAR);
        ...
        //visibility_kernel<<<blocks, threads>>>(d_uvw, d_l, d_m, d_I, d_beam, d_vis, Nuvw, Npix);
        
        // for now the t_start need to be always set to 0
        for (int i = t_start; i < num_timesteps; i++)
        {
...
{   
    /* The visibility kernel proceeds as follows:
